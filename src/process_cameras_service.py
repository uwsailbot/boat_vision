...
def getXDisplacement(x_init, rad, angle):
    return cosd(angle) * rad + x_init

# angle is in CCW degrees from x+ axis
def getYDisplacement(y_init, rad, angle):
...
def process_cams(req):
    ...
    x_final = getXDisplacement(-req.spacing/2.0, dist1, angle1)
    y_final = getYDisplacement(0, dist1, angle1)
...
if __name__ == '__main__':
    ...
    s = rospy.Service('process_cameras', ProcessCameras, process_cams)
    rospy.spin()
